...
async def test_helloworld_test(agent_sdk):
    ...
    request = agent_api_server.TaskRequest(prompt="hello", token_limit = 0, llm_model_name="", input_files=[],
    context_id="")
    responds = []
    ...
    assert responds[len(responds) - 1]['step_type'] == agent_api_server.StepResponseType.OnFinalAnswer
    assert (
